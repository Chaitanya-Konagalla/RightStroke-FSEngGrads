...
export class UnitconversionService {
  ...
  private calcuated_result=new Subject<number>();
  valuetocomponent=this.calcuated_result.asObservable()
  ...
  caluclate1(from:string,to:string,value:number)
  {
    ...
    {
      this.calcuated_result.next(this.F_to_c(value))
    }
    else if(to==='Farenheit' &&  from==='Celsius')
    {
      this.calcuated_result.next(this.C_to_f(value))
    }
    else if(to==='Kelvin' && from==='Celsius')
    {
      this.calcuated_result.next(this.C_to_k(value))
    }
    else if(to==='Celsius' && from==='Kelvin')
    {
      this.calcuated_result.next(this.K_to_c(value))
    }
    else if(to==='Kelvin' && from==='Farenheit')
    {
      this.calcuated_result.next(this.F_to_k(value))
    }
    else if(to==='Farenheit' && from==='Kelvin')
    {
      this.calcuated_result.next(this.K_to_f(value))
    }
    else
    {
      this.calcuated_result.next(value)
    }
  ...
  caluclate2(from:number,to:string,value:number)
  {
    ...
    this.calcuated_result.next(this.conversion[from][to]*value)
